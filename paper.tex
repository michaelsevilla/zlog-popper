...
explain that the state space is large, this is hard for humans.
configuration, software versions, hardware versions, interface selections, and
future features.

\section{Programming Model}

Describe big picture as if we are going to introduce something like bloom for
describing everything.

], Bloom is purely declarative: the syntax of a program contains the full
spec- ification of its semantics, and there is no need for the programmer to
understand or reason about the behavior of the evaluation engine. Bloom is
based on a formal temporal logic called Dedalu

We use a subset of bloom as a basis for defining interface.

Although we have not chosen to use the most powerful parts of the bloom (eg
task coordination and consistency)
language we have chosen to start with a powerful abstraction because we plan
to take this to 11 (see future work).
