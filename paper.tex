...
non-administrative users, the under review inclusion of a mechanism for dynamically
...
implementation against future hardware and software upgrades.
...
addressed portability concerne that arise from defining interfaces
...
high-performance shared-log based on the CORFU protocol can be built, as well
...
using the CORFU system and an existing Ceph-specific interface as examples.
We conclude by discussing the optimization opportunities that we can realize
using analysis of interfaces. I hope you enjoy the show.
...
%  \begin{subfigure}[b]{.47\linewidth}
%      \centering
%      \includegraphics[width=1.0\linewidth]{experiments/basic-cls-overhead/output.1024.soft.reset.png}
%      \caption{1hr,1k,cls\_wr,jewel}
%      \label{fig:cls_wr_jewel}
%  \end{subfigure}
%  \begin{subfigure}[b]{.47\linewidth}
%      \centering
%      \includegraphics[width=1.0\linewidth]{experiments/librados-sweep/firefly_ssd.png}
...
software, system tunables, and hardware parameters that affect the performance 
of ZLog. 
...
2016) - the newest stable release and Firefly (May 2014)- long term support
version that introduced cache tiering and erasure coding.
Figure~\ref{fig:jewel_v_firefly_v_es} shows the behavior and performance of the
benchmark for 128b, 1KB, and 4KB objects.

For all graphs,

That figure demonstrates that the 
