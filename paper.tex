...
design space, and we will then winnow down this space using a set of targetted
...
\begin{enumerate}
    \item {\bf Entry addressing.} We refer to the method by which a client
        ...
        but adds complexity to both the client and the object interface which
...
In order to optimize this process we will iteratively winnow down the space
through a series of targetted experiments.
...
Therefore we first explore the costs of the design space by restricting the
...
reached due to reduced overall throughput of the \emph{11kv} strategy.
...
imposes a large overhead. This is unfortuante because the key-value interface
...
offered by each approach. The third column \emph{entry size} shown in
Table~\ref{tab:pd-map} lists if a particular strategy supports storage of
...
log entries to objects provides flexibilty and the best write performance in
...
%\begin{figure}
%\centering
%\includegraphics[width=\columnwidth]{experiments/basic-cls-rand-read/output.read.60min.1k.png}
%\caption{1hr, 1k, read}
%\label{fig:vanilla_rd_jewel}
%\end{figure}

This result is corroborated by considering the read performance for each
strategy as well. Figure~\ref{fig:vanilla_rd_jewel} shows the expected random
read performance from a log containing 1K entries in which an addressing
strategy that stores log entries in the bytestream has the best performance.
